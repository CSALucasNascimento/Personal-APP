...
/**
 * Consolidate a list of Tags
 */
exports.consolidate = function(req, res) {

  var tagsList = req.body;
  var tagsObj = new Array();

  async.each(tagsList, function (fTag, callback) {
    Tag.find({ 'name': fTag.name }).exec(function (err, rTag) {
      if (rTag.length) {
        tagsObj.push(rTag);
        callback;
      } else {
        var tag = new Tag(fTag);
        tag.user = req.user;
        tag.save(function(err) {
          if (err) {
            return res.status(400).send({
