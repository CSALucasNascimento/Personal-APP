...
exports.list = function(req, res) {
  ...
  Category.find().sort('-created').populate('user', 'displayName').populate('image', 'url').populate('icon', 'url').exec(function(err, categories) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.categoryByID = function(req, res, next, id) {
  ...
  Category.findById(id).populate('user', 'displayName').populate('image', 'url').populate('icon', 'url').populate('parentCategory', 'name').exec(function (err, category) {
    if (err) {
      return next(err);
    } else if (!category) {
